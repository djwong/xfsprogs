...
const field_t	rmapxbt_crc_hfld[] = {
	{ "", FLDT_RMAPXBT_CRC, OI(0), C1, 0, TYP_NONE },
	{ NULL }
};
...
const field_t	rmapxbt_crc_flds[] = {
	{ "magic", FLDT_UINT32X, OI(OFF(magic)), C1, 0, TYP_NONE },
	{ "level", FLDT_UINT16D, OI(OFF(level)), C1, 0, TYP_NONE },
	{ "numrecs", FLDT_UINT16D, OI(OFF(numrecs)), C1, 0, TYP_NONE },
	{ "leftsib", FLDT_AGBLOCK, OI(OFF(u.s.bb_leftsib)), C1, 0, TYP_RMAPXBT },
	{ "rightsib", FLDT_AGBLOCK, OI(OFF(u.s.bb_rightsib)), C1, 0, TYP_RMAPXBT },
	{ "bno", FLDT_DFSBNO, OI(OFF(u.s.bb_blkno)), C1, 0, TYP_RMAPXBT },
	{ "lsn", FLDT_UINT64X, OI(OFF(u.s.bb_lsn)), C1, 0, TYP_NONE },
	{ "uuid", FLDT_UUID, OI(OFF(u.s.bb_uuid)), C1, 0, TYP_NONE },
	{ "owner", FLDT_AGNUMBER, OI(OFF(u.s.bb_owner)), C1, 0, TYP_NONE },
	{ "crc", FLDT_CRC, OI(OFF(u.s.bb_crc)), C1, 0, TYP_NONE },
	{ "recs", FLDT_RMAPBTREC, btblock_rec_offset, btblock_rec_count,
	  FLD_ARRAY|FLD_ABASE1|FLD_COUNT|FLD_OFFSET, TYP_NONE },
	{ "keys", FLDT_RMAPXBTKEY, btblock_key_offset, btblock_key_count,
	  FLD_ARRAY|FLD_ABASE1|FLD_COUNT|FLD_OFFSET, TYP_NONE },
	{ "ptrs", FLDT_RMAPBTPTR, btblock_ptr_offset, btblock_key_count,
	  FLD_ARRAY|FLD_ABASE1|FLD_COUNT|FLD_OFFSET, TYP_RMAPXBT },
	{ NULL }
};
#undef OFF
...
#define	KOFF(f)	bitize(offsetof(struct xfs_rmapx_key, rm_ ## f))

#define RMAPXBK_STARTBLOCK_BITOFF	0
#define RMAPXBK_OWNER_BITOFF		(RMAPXBK_STARTBLOCK_BITOFF + RMAPBT_STARTBLOCK_BITLEN)
#define RMAPXBK_ATTRFLAG_BITOFF		(RMAPXBK_OWNER_BITOFF + RMAPBT_OWNER_BITLEN)
...
const field_t	rmapxbt_key_flds[] = {
	...
	{ "offset", FLDT_RFILEOFFD, OI(RMAPXBK_OFFSET_BITOFF), C1, 0, TYP_NONE },
	{ "attrfork", FLDT_RATTRFORKFLG, OI(RMAPXBK_ATTRFLAG_BITOFF), C1, 0,
	  TYP_NONE },
